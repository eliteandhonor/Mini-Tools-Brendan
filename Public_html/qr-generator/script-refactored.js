...
class QRGenerator {
    ...
    async createQRCode(canvas, text, options) {
        ...
        console.log('Attempting QR generation with text:', text);
        console.log('QRCode library available:', typeof QRCode !== 'undefined');
        
        // Try main QRCode library first
        if (typeof QRCode !== 'undefined' && QRCode.CorrectLevel) {
            return this.generateWithMainLibrary(canvas, text, options, ctx);
        }
        
        // Fall back to SimpleQRCode
        if (typeof SimpleQRCode !== 'undefined') {
            return this.generateWithFallback(canvas, text, options);
        }
    ...
    async generateWithMainLibrary(canvas, text, options, ctx) {
        ...
        try {
            ...
            if (generatedImg && generatedImg.complete) {
                console.log('✅ QR image found, drawing to canvas');
    ...
    async generateWithFallback(canvas, text, options) {
        ...
        return SimpleQRCode.toCanvas(canvas, text, {
            width: options.width,
            height: options.height,
            color: {
